...
classesRouter.get('/', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  const withDetails = await Promise.all(classes.map(async (c: any) => {
    const sections = await query(`
      SELECT s.*, r.room_number, r.floor as room_floor, b.name as default_batch_name, b.code as default_batch_code
      FROM sections s
      LEFT JOIN rooms r ON s.room_id = r.id
      LEFT JOIN batches b ON s.default_batch_id = b.id
      WHERE s.class_id = $1
      ORDER BY s.name ASC
    `, [c.id]);
...
// 5c. Set/change/clear a section's default batch (a safety-net suggestion —
// registration still lets each student's batch be picked/overridden).
classesRouter.put('/sections/:sectionId/default-batch', authenticate, requireRoles('ADMIN', 'PRINCIPAL'), async (req: AuthRequest, res: Response) => {
  const { sectionId } = req.params;
  ...
  await execute(`UPDATE sections SET default_batch_id = $1 WHERE id = $2`, [batch_id || null, sectionId]);
  await logAudit(req, 'SECTION_DEFAULT_BATCH_SET', 'sections', sectionId, { batch_id: batch_id || null });
  return res.json({ success: true, message: batch_id ? 'Default batch assigned.' : 'Default batch cleared.' });
});
