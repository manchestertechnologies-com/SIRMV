...
// 14. Board exam marks — list of cycles attempted (Cycle 1/2/3), with
// total marks / percentage / section rank / overall rank, so the student
// and parent can see exactly where they stand — same shape as the
// theory/competitive lists above, computed from percentage of marks
// obtained across the cycle's subjects.
studentsRouter.get('/:id/marks/board', authenticate, async (req: AuthRequest, res: Response) => {
  const { id } = req.params;

  const exams = await query<any>(`
    SELECT DISTINCT e.id as exam_id, e.name as exam_name, e.academic_year, e.exam_type, e.start_date
    FROM exams e
    JOIN exam_subjects es ON es.exam_id = e.id
    JOIN student_marks sm ON sm.exam_subject_id = es.id
...
// 14b. Board marks card for one cycle — subject-wise marks + section rank +
// overall rank + total + percentage (reuses the same builder as theory/competitive).
studentsRouter.get('/:id/marks/board/:examId', authenticate, async (req: AuthRequest, res: Response) => {
  const { id, examId } = req.params;
  return buildMarksCard(id, examId, res);
});
