...
export const ClassesPage: React.FC = () => {
  ...
  const assignDefaultBatch = async (sectionId: string, batchId: string) => {
    try {
      await apiFetch(`/classes/sections/${sectionId}/default-batch`, {
        method: 'PUT',
          ...
          <div key={c.id} className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-3">
            ...
            <div className="flex flex-col gap-2">
              {c.sections.map((s: any) => (
                <div key={s.id} className="flex flex-col gap-1.5 px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[11px]">
                  ...
                  <div className="flex items-center gap-1.5 px-2 py-1 bg-emerald-50/60 border border-emerald-100 rounded-md">
                    ...
                    <select
                      value={s.default_batch_id || ''}
                      onChange={(e) => assignDefaultBatch(s.id, e.target.value)}
