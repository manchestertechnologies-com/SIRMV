...
import { Plus, X, Users, Target, Trash2, ArrowLeft, Layers, Clock } from 'lucide-react';
...
const BatchDetail: React.FC<{ batchId: string; branchId: string; onBack: () => void }> = ({ batchId, branchId, onBack }) => {
  ...
  // Group class/sections by class for display — one grouping for sections
  // actually registered into this batch (real students), another for
  // sections that just have this set as their Default Batch (upcoming/
  // expected, ahead of any registration).
  const groupByClass = (rows: any[]) => {
    const byClass = new Map<string, { class_name: string; sections: string[] }>();
    for (const row of rows) {
      if (!byClass.has(row.class_id)) byClass.set(row.class_id, { class_name: row.class_name, sections: [] });
      byClass.get(row.class_id)!.sections.push(row.section_name);
    }
    return byClass;
  };
  const actualByClass = groupByClass(data.coverage || []);
  const upcomingByClass = groupByClass(data.defaultSections || []);
  ...
  return (
    <div className="space-y-5 max-w-3xl mx-auto">
      ...
      {/* Upcoming coverage — sections that have this batch set as their
          Default Batch on the Classes page, ahead of any registration. */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-3">
        <h3 className="font-bold text-slate-800 text-xs uppercase tracking-wider flex items-center gap-1.5">
          <Clock className="w-3.5 h-3.5 text-amber-500" />
          Upcoming Sections (Default Batch)
        </h3>
        {upcomingByClass.size === 0 ? (
          <p className="text-xs text-slate-400">No section has this set as its Default Batch yet. Set it from the Classes page.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {Array.from(upcomingByClass.entries()).map(([classId, info]) => (
              <div key={classId} className="border border-slate-100 rounded-xl p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {info.sections.map((s) => (
                    <span key={s} className="px-2.5 py-1 rounded-lg text-[11px] font-semibold bg-amber-50 border border-amber-200 text-amber-800">
                      Sec {s}
                    </span>
                  ))}
                </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-3">
        <h3 className="font-bold text-slate-800 text-xs uppercase tracking-wider flex items-center gap-1.5">
          ...
          Classes / Sections Covered (Registered Students)
        </h3>
        {actualByClass.size === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {Array.from(actualByClass.entries()).map(([classId, info]) => (
              <div key={classId} className="border border-slate-100 rounded-xl p-3">
                <div className="text-xs font-bold text-slate-700 mb-1.5">{info.class_name}</div>
                <div className="flex flex-wrap gap-2">
                  {info.sections.map((s) => (
