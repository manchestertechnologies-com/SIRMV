...
const BatchDetail: React.FC<{ batchId: string; branchId: string; onBack: () => void }> = ({ batchId, branchId, onBack }) => {
  ...
  const load = () => {
    Promise.all([
      apiFetch<any>(`/batches/${batchId}`),
      apiFetch<any>(`/classes?branch_id=${branchId}`).catch(() => ({ classes: [] }))
    ]).then(([batchRes, classesRes]) => {
      setData(batchRes);
  ...
  const coveredSectionIds = new Set((data.coverage || []).map((c: any) => c.section_id));

  const toggleCoverage = async (classId: string, sectionId: string) => {
    setSavingSectionId(sectionId);
    try {
      if (coveredSectionIds.has(sectionId)) {
        const existing = data.coverage.find((c: any) => c.section_id === sectionId);
        await apiFetch(`/batches/${batchId}/coverage/${existing.id}`, { method: 'DELETE' });
      } else {
        await apiFetch(`/batches/${batchId}/coverage`, {
          method: 'POST',
  ...
  return (
    <div className="space-y-5 max-w-3xl mx-auto">
      ...
      {/* Class/Section coverage — which sections this batch applies to */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-3">
        ...
        {(data.coverage || []).length === 0 && (
          <p className="text-xs text-slate-400">No class/section assigned to this batch yet — check the boxes below.</p>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {classes.map((c) => (
            <div key={c.id} className="border border-slate-100 rounded-xl p-3">
              <div className="text-xs font-bold text-slate-700 mb-1.5">{c.name}</div>
              <div className="flex flex-wrap gap-2">
                {c.sections.map((s: any) => {
                  const covered = coveredSectionIds.has(s.id);
                  return (
                    <label
                      key={s.id}
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-semibold cursor-pointer border ${
                        covered ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-slate-50 border-slate-200 text-slate-500'
                      } ${savingSectionId === s.id ? 'opacity-50' : ''}`}
                    >
                      <input
                        type="checkbox"
                        checked={covered}
                        disabled={savingSectionId === s.id}
                        onChange={() => toggleCoverage(c.id, s.id)}
                        className="hidden"
                      />
                      Sec {s.name}
                    </label>
                ...
                {c.sections.length === 0 && <span className="text-[11px] text-slate-300">No sections</span>}
              </div>
            </div>
          ))}
        </div>
