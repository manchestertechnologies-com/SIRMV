...
batchesRouter.get('/:id', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  const coverage = await query(`
    SELECT bc.id, bc.class_id, bc.section_id, c.name as class_name, sec.name as section_name
    FROM batch_coverage bc
    JOIN classes c ON bc.class_id = c.id
    JOIN sections sec ON bc.section_id = sec.id
    WHERE bc.batch_id = $1
    ORDER BY c.name ASC, sec.name ASC
  `, [id]);

  return res.json({ batch, students, coverage });
});

// 2b. Add a class/section this batch covers
batchesRouter.post('/:id/coverage', authenticate, requireRoles('ADMIN', 'PRINCIPAL'), async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  ...
  const coverageId = 'bcov-' + crypto.randomUUID();
  await execute(`
    INSERT INTO batch_coverage (id, batch_id, class_id, section_id)
    VALUES ($1, $2, $3, $4)
    ON CONFLICT (batch_id, section_id) DO NOTHING
  `, [coverageId, id, class_id, section_id]);

  await logAudit(req, 'BATCH_COVERAGE_ADDED', 'batch_coverage', coverageId, { batch_id: id, class_id, section_id });
  return res.status(201).json({ success: true, message: 'Class/section added to this batch\'s coverage.' });
});

// 2c. Remove a class/section this batch covers
batchesRouter.delete('/:id/coverage/:coverageId', authenticate, requireRoles('ADMIN', 'PRINCIPAL'), async (req: AuthRequest, res: Response) => {
  const { coverageId } = req.params;
  await execute(`DELETE FROM batch_coverage WHERE id = $1`, [coverageId]);
  await logAudit(req, 'BATCH_COVERAGE_REMOVED', 'batch_coverage', coverageId, {});
  return res.json({ success: true, message: 'Class/section removed from this batch\'s coverage.' });
});
