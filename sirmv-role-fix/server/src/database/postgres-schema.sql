...
-- Optional default/typical batch for a SECTION (e.g. "1 PUC Section A"
-- defaults to "NEET Batch"), set from the Classes admin page as a
-- safety-net default that can still be overridden per-student at
-- registration time. Added here, after `batches`, since `sections` is
-- declared earlier in this file and can't forward-reference it in its
-- own CREATE TABLE.
...
-- 18. Announcements — the Noticeboard's underlying storage (server/src/routes/announcements.ts
-- already reads/writes this; it was missing from the schema file, which is why Announcements/
-- Notifications silently failed for every role, not just the ones flagged in this audit).
CREATE TABLE IF NOT EXISTS announcements (
  ...
  target_roles VARCHAR(255), -- comma-separated role list, NULL/empty = everyone in the branch
...
-- 19. Notifications — the Navbar bell's per-user inbox (server/src/routes/notifications.ts).
...
-- 20. Web Push (VAPID) device subscriptions, one row per browser/device.
...
-- 21. College Calendar — holidays, exams, events, meetings visible to staff/students.
CREATE TABLE IF NOT EXISTS calendar_events (
  id VARCHAR(64) PRIMARY KEY,
  branch_id VARCHAR(64) NOT NULL REFERENCES branches(id) ON DELETE CASCADE,
  title VARCHAR(255) NOT NULL,
  description TEXT,
  event_date VARCHAR(32) NOT NULL, -- YYYY-MM-DD
  end_date VARCHAR(32), -- optional, for multi-day events/holidays
...
-- 22. Student Counselling — a class teacher's counselling session log for a student.
...
-- 23. Student Discipline — a hostel warden's (or staff) incident log for a student.
CREATE TABLE IF NOT EXISTS discipline_records (
  id VARCHAR(64) PRIMARY KEY,
  branch_id VARCHAR(64) NOT NULL REFERENCES branches(id) ON DELETE CASCADE,
  student_id VARCHAR(64) NOT NULL REFERENCES student_profiles(id) ON DELETE CASCADE,
  incident_date VARCHAR(32) NOT NULL,
  category VARCHAR(64), -- e.g. Late Return, Ragging, Property Damage, Curfew Violation
...
-- 24. Floor Issues — a floor in-charge's maintenance/operational issue reports
-- (broken projector, AC not working, etc.), triaged by admin/principal.
