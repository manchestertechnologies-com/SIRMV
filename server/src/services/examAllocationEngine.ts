 ...
 *  2. Other rooms belonging to the same batch/stream (not modeled
 *     separately here — falls through to priority 3, which already
 *     prefers whatever's available).
 *  3. Other available rooms, smallest-fit-first, to minimize unused seats
 *     and the number of extra rooms needed.
 */
export function selectRoomsAutomatic(
  ...
  // Rooms not yet touched by any batch, sorted smallest-capacity-first so
  // automatic overflow allocation doesn't waste a large room on a small
  // remainder ("minimize unused seating capacity").
  const genericPool = () =>
    availableRooms
      .filter((r) => r.isAvailableForExams && !alreadyUsedRoomIds.has(r.roomId))
      .sort((a, b) => roomCapacity(a) - roomCapacity(b));

  for (const batch of batches) {
    let remaining = batch.studentIds.length;
    if (remaining === 0) continue;

    // Priority 1: the batch's own regular classroom.
    ...
    // Priority 3 (overflow): fill from the smallest available room that
    // still has space, reusing rooms already opened by this plan first so
    // we don't spread students across more rooms than necessary.
    while (remaining > 0) {
      const openRoom = plan
