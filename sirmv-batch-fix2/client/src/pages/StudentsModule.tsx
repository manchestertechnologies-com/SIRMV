                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const cls = options.classes.find((c: any) => c.id === classId);
                      // Pre-fill the batch from the class's default (safety-net),
                      // but leave it editable — the admin can still override it.
                      setCreateForm({
                        ...createForm,
                        class_id: classId,
                        section_id: '',
                        batch_id: (cls as any)?.default_batch_id || createForm.batch_id
                      });
                    }}
                    className="w-full bg-slate-50 border border-[#ded9cf] rounded-xl px-3 py-2 text-xs text-slate-900 outline-none"
                  >
