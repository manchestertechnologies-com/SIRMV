...
classesRouter.get('/', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  const classes = await query(`
    SELECT c.*, b.name as default_batch_name, b.code as default_batch_code
    FROM classes c
    LEFT JOIN batches b ON c.default_batch_id = b.id
    WHERE c.branch_id = $1
    ORDER BY c.name ASC
  `, [branchId]);
...
// 3b. Set/change/clear a class's default batch (a safety-net suggestion —
// registration still lets each student's batch be picked/overridden).
classesRouter.put('/:id/default-batch', authenticate, requireRoles('ADMIN', 'PRINCIPAL'), async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  ...
  await execute(`UPDATE classes SET default_batch_id = $1 WHERE id = $2`, [batch_id || null, id]);
  await logAudit(req, 'CLASS_DEFAULT_BATCH_SET', 'classes', id, { batch_id: batch_id || null });
